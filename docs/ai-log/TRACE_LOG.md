...
| 35 | 2026-02-14T03:34:00Z | Security Remediation | Rewrite git history to purge historical `.env.example` leaks | Sanitized history + restored clean `.env.example` template | `security:scan` moved from leak warnings to pass state |
| 36 | 2026-02-14T03:38:00Z | Release Freeze | Run `verify:ci` and `release:prepare` on frozen proof tx | Fresh `DORAHACKS_SUBMISSION_FINAL.md` + verified event artifact | Submission pack is synchronized on one canonical contract+tx pair |
