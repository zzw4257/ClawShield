...
const publicV2Root = path.resolve(remotionRoot, "public/v2");
const outCasesRoot = path.resolve(publicV2Root, "cases");
const outAiRoot = path.resolve(publicV2Root, "ai-log/screenshots");
const outKeyframesRoot = path.resolve(publicV2Root, "keyframes");
const outEvidenceRoot = path.resolve(publicV2Root, "evidence");
...
function copyCaseArtifacts(manifest: AssetManifest): void {
  ...
  for (const caseId of caseDirs) {
    ...
    for (const name of files) {
      ...
      fs.copyFileSync(from, to);
      copied.push(`v2/cases/${caseId}/${name}`);
    }
...
function copyAiScreenshots(manifest: AssetManifest): void {
  ...
  for (const name of files) {
    ...
    fs.copyFileSync(from, to);
    manifest.copied.aiScreenshots.push(`v2/ai-log/screenshots/${name}`);
  }
}
...
function copyKeyframes(manifest: AssetManifest): void {
  ...
  for (const name of files) {
    ...
    fs.copyFileSync(from, to);
    manifest.copied.keyframes.push(`v2/keyframes/${name}`);
  }
}
...
function copyEvidence(manifest: AssetManifest): void {
  ...
  for (const rel of evidenceFiles) {
    ...
    if (copyIfExists(from, to)) {
      manifest.copied.evidence.push(`v2/evidence/${path.basename(rel)}`);
    }
  }
}
...
function main(): void {
  ...
  ensureDir(path.resolve(publicV2Root, "audio"));
  ...
  console.log(`v2 assets prepared at: ${path.relative(projectRoot, publicV2Root)}`);
