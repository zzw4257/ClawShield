...
import type { VideoV2CaseSnapshot, VideoV2Manifest, V2RiskLevel } from "../src/v2/types";
...
function asRiskLevel(value: unknown, fallback: V2RiskLevel): V2RiskLevel {
...
function caseSnapshotFromArtifacts(item: LockedCase): VideoV2CaseSnapshot {
  ...
  return {
    ...
    txHash,
    evidence: {
      homeImage: `v2/cases/${item.id}/home.png`,
      auditImage: `v2/cases/${item.id}/audit.png`,
      fingerprintImage: `v2/cases/${item.id}/fingerprint.png`,
      flowJson: `v2/cases/${item.id}/flow.json`,
      reportJson: `v2/cases/${item.id}/report.json`,
      attestJson: `v2/cases/${item.id}/attest.json`
    }
  };
}

function writeGeneratedTs(data: VideoV2Manifest): void {
  const outPath = path.resolve(remotionRoot, "src/v2/generated.ts");
  const payload = JSON.stringify(data, null, 2);

  const file = `import { VideoV2Manifest } from "./types";\n\n// Auto-generated by remotion/scripts/sync-video-v2-data.ts\n// Do not edit manually.\nexport const VIDEO_V2_DATA: VideoV2Manifest = ${payload};\n`;
...
function main(): void {
  ...
  const submissionConfig = readOptionalJson<JsonObject>(
    path.resolve(projectRoot, "docs/submission.config.json")
  ) || { };
  ...
  const videoUrl =
    rawVideoUrl.length === 0
      ? "media/video/clawshield-demo-v2.mp4"
      : rawVideoUrl.includes("clawshield-demo.mp4") && !rawVideoUrl.includes("clawshield-demo-v2.mp4")
        ? rawVideoUrl.replace("clawshield-demo.mp4", "clawshield-demo-v2.mp4")
        : rawVideoUrl;

  const data: VideoV2Manifest = {
    ...
    aiEvidence: {
      ...
      screenshots: listPublicFiles("v2/ai-log/screenshots", /\.(png|jpg|jpeg|webp)$/i),
      keyframes: listPublicFiles("v2/keyframes", /\.(png|jpg|jpeg|webp)$/i),
